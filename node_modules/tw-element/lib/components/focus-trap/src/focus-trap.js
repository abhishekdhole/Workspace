...
const _sfc_main = vue.defineComponent({
  ...
  setup(props, { emit }) {
    ...
    vue.onMounted(() => {
      ...
      if (trapContainer) {
        ...
        if (!isPrevFocusContained) {
          ...
          event.on(trapContainer, tokens.FOCUS_ON_MOUNT, focusOnMount);
      ...
      vue.watch(() => props.trapped, (trapped) => {
        if (trapped) {
          event.on(document, "focusin", onFocusIn);
          event.on(document, "focusout", onFocusOut);
        } else {
          event.off(document, "focusin", onFocusIn);
    ...
    vue.onBeforeUnmount(() => {
      const trapContainer = vue.unref(forwardRef);
      if (trapContainer) {
        event.off(trapContainer, tokens.FOCUS_ON_MOUNT, focusOnMount);
        const unmountEvent = new Event(tokens.FOCUS_ON_UNMOUNT, tokens.FOCUS_ON_MOUNT_OPTS);
        event.on(trapContainer, tokens.FOCUS_ON_UNMOUNT, focusOnUnmount);
        ...
        event.off(trapContainer, tokens.FOCUS_ON_UNMOUNT, focusOnUnmount);
